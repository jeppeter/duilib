...
void CRenderEngine::DrawHtmlText(HDC hDC, CPaintManagerUI* pManager, RECT& rc, LPCTSTR pstrText, DWORD dwTextColor, RECT* prcLinks, CDuiString* sLinks, int& nLinkRects, int iDefaultFont, UINT uStyle)
{
    ...
    int iLinkIndex = 0;
	int cxLine = 0;
    int cyLine = pTm->tmHeight + pTm->tmExternalLeading + (int)(ADDR_T)(aPIndentArray.GetAt(aPIndentArray.GetSize() - 1));
    int cyMinHeight = 0;
    int cxMaxWidth = 0;
    POINT ptLinkStart = { 0 };
    bool bLineEnd = false;
    ...
    while( *pstrText != _T('\0') ) {
        if( pt.x >= rc.right || *pstrText == _T('\n') || bLineEnd ) {
            ...
            if( pt.y > rc.bottom && bDraw ) 
				break;
            ptLinkStart = pt;
            cyLine = pTm->tmHeight + pTm->tmExternalLeading + (int)(ADDR_T)(aPIndentArray.GetAt(aPIndentArray.GetSize() - 1));
            if( pt.x >= rc.right )
				break;
        }
            ...
            case _T('a'):  // Link
                {
                    ...
                    //}
                    aColorArray.Add((LPVOID)((ADDR_T)clrColor));
                    ...
                    if( pFontInfo->bUnderline == false ) {
                        ...
                        cyLine = MAX(cyLine, pTm->tmHeight + pTm->tmExternalLeading + (int)(ADDR_T)(aPIndentArray.GetAt(aPIndentArray.GetSize() - 1)));
                    }
                    ptLinkStart = pt;
                    bInLink = true;
                }
            ...
            case _T('c'):  // Color
                {
                    ...
                    aColorArray.Add((LPVOID)((ULONG64)clrColor));
                    ::SetTextColor(hDC, RGB(GetBValue(clrColor), GetGValue(clrColor), GetRValue(clrColor)));
                }
                break;
            case _T('f'):  // Font
						...
						if( pImageInfo ) {
                            ...
                            if( pt.x + iWidth > rc.right && pt.x > rc.left && (uStyle & DT_SINGLELINE) == 0 ) {
                                ...
                                if( bDraw && bLineDraw ) {
									...
									iVAlign = DT_BOTTOM;
									if (aVAlignArray.GetSize() > 0) iVAlign = (UINT)(ADDR_T)aVAlignArray.GetAt(aVAlignArray.GetSize() - 1); 
            ...
            case _T('p'):  // Paragraph
                {
                    ...
                    aPIndentArray.Add((LPVOID)((ADDR_T)cyLineExtra));
			...
			case _T('v'):  // Vertical Align
				{
					...
					aVAlignArray.Add((LPVOID)((ADDR_T)iVAlign));
				}
				break;
            case _T('r'):  // Raw Text
                {
            ...
            case _T('c'):
                {
                    ...
                    if( aColorArray.GetSize() > 0 ) clrColor = (int)(ADDR_T)aColorArray.GetAt(aColorArray.GetSize() - 1);
                    ::SetTextColor(hDC, RGB(GetBValue(clrColor), GetGValue(clrColor), GetRValue(clrColor)));
                }
                break;
            case _T('p'):
